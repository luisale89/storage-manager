import functools


def debug_logger(my_logger):
    '''logger decorator for debug. 
    parameter: my_logger -> specific logger to be used by the decorated function'''
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            args_repr = [repr(a) for a in args]
            kwargs_repr = [f"{k}={v!r}" for k, v in kwargs.items()]
            signature = ", ".join(args_repr + kwargs_repr)
            my_logger.debug(f"executing {func.__name__}({signature})")
            value = func(*args, **kwargs)
            my_logger.debug(f"{func.__name__!r} returned {value!r}")
            return value
        return wrapper
    return decorator


def production_logger(my_logger):
    '''logger decorator for production
    parameter: my_logger -> specific logger to be used by the decorated function
    '''
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            my_logger.info(f'executing {func.__name__}()')
            return func(*args, **kwargs)
        return wrapper
    return decorator